...
export class AwinSource implements ProductSource {
  ...
  // localFile : ingérer depuis un CSV déjà téléchargé (offline/déterministe) plutôt que
  // de re-fetcher l'URL — pratique pour un premier import contrôlé.
  constructor(public readonly merchant: string, private readonly localFile?: string) {}

  private async loadFeed(): Promise<AwinRow[]> {
    let buf: Buffer;
    if (this.localFile) {
      buf = readFileSync(this.localFile);
    } else {
      ...
      if (!feedUrl) throw new Error(`AwinSource: feed_url manquant pour '${this.merchant}' (Create-a-Feed → URL CSV → partner_merchants.feed_url)`);
    ...
    // Gunzip si compressé (magic 1f 8b) — Create-a-Feed compression/gzip.
  ...
  private toInput(r: AwinRow): PartnerProductInput | null {
    // Image pour l'EMBEDDING : on préfère l'original pleine résolution du marchand au proxy
    // Awin (souvent bridé en 200×200) → embedding image bien plus net. Fallbacks ensuite.
    const image = r.merchant_image_url || r.large_image || r.aw_image_url || "";
    const images = [...new Set([image, r.aw_image_url, r.large_image].filter(Boolean))];
    ...
    // Stock : si une colonne le précise et qu'il est nul, on saute (on ne propose que du dispo).
    const stock = (r.in_stock || r.stock_status || "").toLowerCase();
    if (stock === "0" || stock === "out of stock" || stock === "no" || stock === "false") return null;

    // Flag extérieur : le chemin de catégorie contient "Maison et JARDIN" partout → inutilisable.
    // Signal fiable = "extérieur/outdoor/terrasse" dans le nom ou la fin du chemin catégorie.
    const outdoor = /ext[ée]rieur|outdoor|terrasse/i.test(`${name} ${r.merchant_product_category_path ?? ""}`);
    ...
    return {
      ...
      attributes: {
        ...
        // Flag réutilisable (futurs aménagements terrasse) : tapis utilisable en extérieur.
        outdoor,
        platform: "awin",
      },
    };
